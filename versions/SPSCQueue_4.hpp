...
public:
  ...
  ControlBlock head;
  ControlBlock tail;
  alignas(cache_align) mutable std::size_t cached_head { 0 };
  alignas(cache_align) mutable std::size_t cached_tail { 0 };
  alignas(cache_align) T *data;
...
{
  if (index - cached_tail == N)
  {
    std::size_t prev_cached = cached_tail;
    cached_tail = tail.index.load(std::memory_order_acquire);
    return prev_cached == cached_tail;
  }
  else
  {
    return false;
...
{
  if (index == cached_head)
  {
    cached_head = head.index.load(std::memory_order_acquire);
    return index == cached_head;
  }
  else
  {
    return false;
